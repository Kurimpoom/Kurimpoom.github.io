...
DFT正变换公式：$$X(k) = \sum\limits_{n = 0}^{N - 1} {x(n)W_N^{kn}} $$&emsp;&emsp;&emsp;(&nbsp;0&nbsp;≤&nbsp;k&nbsp;≤&nbsp;N&nbsp;-&nbsp;1&nbsp;)&emsp;其中$$W_N^{kn} = {e^{ - j\frac{2\pi }{N}\omega n}}$$



从上面的公式我们可以看出，DFT变换是DTFT变换在【0，2π】区间上的N点等间隔采样，采样间隔为2π/N，详细解释[可参考这里](https://www.cnblogs.com/BitArt/archive/2012/11/24/2786390.html)
...
（1）**频谱混叠**  
  ...
  在x<sub>N</sub>(N) = x(n)R<sub>N</sub>(N)这个过程中会发生频谱泄漏（Rx(N)为矩形序列）：因为R<sub>N</sub>(n)序列的DTFT是一个Sa()函数，时域的乘积对应频域的卷积，因此会发生频谱的泄露。   
 ...
 - #### 参数选择
  ...
  信号的持续时间Tp应满足频率分辨率的要求，即Tp=1/Δf，即长的信号采样持续时间会得到较高的频率分辨率（即Δf越小）<br>
